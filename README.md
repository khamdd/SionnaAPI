...
revision at startup but does not create or alter tables. Docker Compose runs
`alembic upgrade head` automatically through the one-shot `database-migrate`
service.
...
# Show the installed revision and check model/schema consistency
python -m alembic current
python -m alembic check

# Generate a proposed revision after changing backend/models.py
python -m alembic revision --autogenerate -m "describe the schema change"

# Revert one revision only after reviewing its downgrade implementation
python -m alembic downgrade -1
```

To run the migration service manually with Docker:

```powershell
docker compose --env-file .env.docker run --rm database-migrate
```

Always review autogenerated revisions and back up production data before applying
or downgrading migrations. A database created before Alembic must first pass the
guarded `python -m backend.migrations.adopt_legacy_database` adoption command.
