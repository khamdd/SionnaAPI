...
- Scene choosing now lives at `/choose-scene` as a full-page offline Vietnam map
  with a Hà Nội ward search bar, a major-city dropdown, scene-name entry, area
  drawing, preview, and keep/load actions. Typing in the ward search filters
...
- The Choose scene modal uses a Docker-served offline MapLibre/PMTiles map from
  `frontend/public/data/` with `vietnam.pmtiles`, `building-regions.json`,
  regional `vn-buildings-*.pmtiles` archives, and Hà Nội ward data
  (`hanoi-wards.csv` list plus `hanoi-wards.geojson` simplified boundaries,
  generated by `scripts/generate_vietnam_admin_csv.py` and
  `scripts/generate_hanoi_wards_geojson.py` from the Vietnamese provinces
  database). At zoom 13.5+, visible 1-degree building regions are loaded as 3D
  fill-extrusion layers and unloaded when they leave the viewport. Large PMTiles
  assets are ignored by git but checked during the frontend Docker build.
