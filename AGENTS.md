...
- Scene choosing now lives at `/choose-scene` as a full-page offline Vietnam map
  with a nationwide DB-backed ward search bar, a province dropdown, scene-name
  entry, area drawing, preview, and keep/load actions. Typing in the ward
  search queries the Vietnam admin API; choosing a ward flies the map to it,
  draws its simplified boundary with a red border, and takes the ward's
...
- The Choose scene modal uses a Docker-served offline MapLibre/PMTiles map from
  ...
  building regions are loaded as 3D fill-extrusion layers and unloaded when
  they leave the viewport. Large PMTiles assets are ignored by git but checked
...
- Vietnam admin reference data lives in `vietnam_provinces` (34 rows) and
  ...
  boundary as cached GeoJSON. Ward search matches the diacritic-stripped
  query as a prefix or substring, ranking prefix matches first; ordering and
  the limit apply after that ranking. These endpoints require a configured
  database and return 503 in no-database mode. The frontend cutover is done:
  `/choose-scene` loads the province dropdown, debounced ward search scoped
  to the selected province (12 results; the input stays disabled until a
  province is chosen), and per-ward red boundary
  overlays from these endpoints through `frontend/src/api/vietnamAdmin.js`,
  with a small in-memory ward-boundary cache; in no-database mode the picker
  is unavailable and only manual area drawing works on that page. The static
  `hanoi-wards.csv`/`hanoi-wards.geojson` files were removed after the
  cutover, and `frontend/src/utils/wardSearch.js` client-side filtering was
  removed with them.
...
- `frontend/src/hooks/useSceneAntennaDraft.js`: shared scene-scoped antenna draft
  state for Network Coverage, RSRP, SINR, and Throughput, configured with each
  page's enabled controls, active limits, role selections, and storage keys.
- `frontend/src/api.js`: compatibility barrel re-exporting the stable API
  ...
  `auth.js`, `simulations.js`, `jobs.js`, `scenes.js`, `automation.js` for
  configurations and profiles, and `vietnamAdmin.js` for provinces, wards,
  and ward boundaries)
