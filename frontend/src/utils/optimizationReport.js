...
export function buildOptimizationReport({
  ...
  const recommended =
    optimization.recommended_candidate || optimization.best || {};
  ...
  const changeRows =
    (recommended.changes || [])
      ...
      .join("") ||
    `<tr><td colspan="5">The starting antenna settings remain recommended.</td></tr>`;
  const settingIds = [
    ...new Set([
  ...
  const settingRows =
    settingIds
      ...
      .join("") ||
    `<tr><td colspan="3">Full antenna settings are unavailable.</td></tr>`;
  const guardrailRows =
    (recommended.evaluation?.guardrails || [])
      .map(
        (guardrail) =>
  ...
  const alternativeRows =
    (optimization.alternatives || [])
      ...
      .join("") ||
    `<tr><td colspan="5">No alternative setup available.</td></tr>`;
  const mapSection =
    baselineGrid?.cells?.length && recommendedGrid?.cells?.length
      ? `<div class="maps"><figure><figcaption>Starting coverage</figcaption>${gridImage(baselineGrid, "coverage", null, "Starting coverage")}</figure><figure><figcaption>Recommended coverage</figcaption>${gridImage(recommendedGrid, "coverage", null, "Recommended coverage")}</figure><figure><figcaption>Coverage change</figcaption>${gridImage(recommendedGrid, "coverage", baselineGrid, "Coverage change")}</figure></div><p class="legend"><span class="blue">Covered / gained</span><span class="gray">Unchanged / uncovered</span><span class="red">Lost coverage</span></p>`
  ...
  <header class="report-head"><div><p>Network coverage optimization</p><h1>${escapeHtml(scene?.name || "Engineering report")}</h1></div></header>
...
function gridImage(grid, measurement, baselineGrid, label) {
  if (typeof document === "undefined")
    return gridSvg(grid, measurement, baselineGrid);
  const canvas = document.createElement("canvas");
  const cells = grid.cells || [];
  const rows =
    Number(grid.rows) ||
    Math.max(1, ...cells.map((cell) => Number(cell.row) + 1 || 1));
  const cols =
    Number(grid.cols) ||
    Math.max(1, ...cells.map((cell) => Number(cell.col) + 1 || 1));
  ...
  const baseline = new Map(
    (baselineGrid?.cells || []).map((cell, index) => [
      cellKey(cell, index),
      cell,
    ]),
  );
  cells.forEach((cell, index) => {
    const row = Number.isFinite(Number(cell.row))
      ? Number(cell.row)
      : Math.floor(index / cols);
    const col = Number.isFinite(Number(cell.col))
      ? Number(cell.col)
      : index % cols;
    const value =
      measurement === "coverage"
        ? coverageValue(cell)
        : numeric(cell[measurement]);
    const baselineCell = baseline.get(cellKey(cell, index));
    const before =
      measurement === "coverage"
        ? coverageValue(baselineCell)
        : numeric(baselineCell?.[measurement]);
    context.fillStyle = baselineGrid
      ? deltaColor(value, before, measurement)
...
function cellKey(cell, index) {
  return Number.isFinite(Number(cell?.row)) &&
    Number.isFinite(Number(cell?.col))
    ? `${cell.row}:${cell.col}`
    : String(index);
}
...
function coverageValue(cell) {
  if (!cell || cell.overlap_level === "no_coverage") return 0;
  const overlapCount = numeric(cell.overlap_count);
  if (overlapCount !== null) return overlapCount > 0 ? 1 : 0;
  return numeric(cell.sinr_db) === null ? 0 : 1;
}
function measurementColor(measurement, value) {
  if (measurement === "coverage")
    return value === null ? "#cbd3d8" : value ? "#267da8" : "#cbd3d8";
  return value === null
    ? "#cbd3d8"
    : value < -110
      ? "#b84a4a"
...
function deltaColor(after, before, measurement) {
...
function display(value) {
  return typeof value === "number" && Number.isFinite(value)
    ? escapeHtml(Number(value.toFixed(3)))
    : "—";
}
function signed(value) {
  const number = Number(value);
  return Number.isFinite(number)
    ? `${number > 0 ? "+" : ""}${display(number)}`
    : "—";
}
function fieldName(field) {
  return (
    { tilt: "Tilt", tx_power: "Transmit power", azimuth: "Azimuth" }[field] ||
    field ||
    "Setting"
  );
}
function settingsText(settings) {
  return settings
    ? `Tilt ${display(settings.tilt)}°, power ${display(settings.tx_power)} dBm, azimuth ${display(settings.azimuth)}°`
    : "Unavailable";
}
function objectiveName(objective) {
  if (objective.kind === "threshold_area")
    return `${String(objective.measurement).replaceAll("_", " ")} threshold area`;
  if (objective.kind === "percentile")
    return `P${objective.percentile} ${String(objective.measurement).replaceAll("_", " ")}`;
  return METRIC_LABELS[objective.metric] || objective.metric || "Objective";
}
function objectiveTarget(objective) {
  return `${objective.operator || ""} ${display(objective.target)}`.trim();
}
  ...
  return (
    {
      ...
      refinement_stalled: "local refinement stopped improving",
